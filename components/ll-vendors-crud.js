...
export class LlVendorsCrud extends HTMLElement {
  constructor() {
    super();
  ...
  async refresh() {
    const items = await db.getAll('catalog_vendors');
  ...
  render() {
    this.shadowRoot.innerHTML = `
      ${sharedStylesTag()}
      <section class="crud" aria-label="${t('vendors')}">
        <div class="crud-header">
          <h2 class="crud-title">${t('vendors')}</h2>
          <p class="crud-subtitle">${t('vendors.subtitle')}</p>
        </div>

        <div class="crud-add">
          <label class="visually-hidden" for="vendor-new">${t('add.new')}</label>
          <input id="vendor-new" type="text" placeholder="${t('vendors.add.placeholder')}" autocomplete="off" />
          <button class="btn primary" id="vendor-add-btn" type="button">${t('add.new')}</button>
        </div>
  ...
  async _onClick(e) {
    const addBtn = e.target?.id === 'vendor-add-btn' ? e.target : null;
    if (addBtn) {
      const input = this.shadowRoot.getElementById('vendor-new');
  ...
  async _upsert(name) {
    ...
    const items = await db.getAll('catalog_vendors');
    const exists = (items || []).some((v) => normalizeKey(v.normalizedName || v.name) === key);
    if (exists) {
      showToast(t('catalog.duplicate'), { variant: 'warning' });
      return;
    }
    await db.add('catalog_vendors', { name, normalizedName: key, createdAt: new Date() });
    await this.refresh();
    this._emitChanged();
  }
  ...
  async _rename(id, nextName) {
    ...
    const items = await db.getAll('catalog_vendors');
    const current = (items || []).find((v) => String(v.id) === String(id));
    if (!current) return;
    const exists = (items || []).some((v) => String(v.id) !== String(id) && normalizeKey(v.normalizedName || v.name) === key);
    if (exists) {
      showToast(t('catalog.duplicate'), { variant: 'warning' });
      return;
    }
    await db.put('catalog_vendors', { ...current, name: nextName, normalizedName: key });
  }
  ...
  async _delete(id) {
    ...
    const inUse = (movements || []).some((m) => normalizeKey(m.vendor) === normalizeKey(current.name));
    if (inUse) {
    ...
    await db.delete('catalog_vendors', Number(current.id));
    await this.refresh();
    this._emitChanged();
  }
...
customElements.define('ll-vendors-crud', LlVendorsCrud);
